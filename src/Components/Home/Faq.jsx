...
export default function Faq() {
  ...
  const items = texts.map((text, index) => ({
    ...
    label: (
      <p className="text-white lg:text-[20px] text-[16px] font-lato">
        {text.label}
      </p>
    ),
    children: (
      <p className="text-white lg:text-[16px] text-[14px] font-lato">
        {text.text}
      </p>
    ),
  }));
  ...
  return (
    <section className="faq bg-[rgb(30,31,39)] section ">
      <div className="faq-container 2xl:w-[1300px] xl:w-[1250px] lg:w-[950px] md:w-[750px] sm:w-[540px] custom:w-[380px] w-[300px]  mx-auto">
        ...
        <Collapse className="custom-collapse pt-4 pb-8" ghost items={items} />
      </div>
    </section>
  );
}
