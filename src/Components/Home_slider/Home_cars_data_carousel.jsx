        ...
        {Cars?.map((car, index) => (
          <div key={index}>
            <div className="flex flex-row justify-between mt-28">
              <h1 className="text-white font-[600] md:text-[30px] text-[20px] ">
                {car?.category?.name_en}
              </h1>
              <h1
                className="text-white font-[600] md:text-[20px] text-[17px] hover:mr-2"
                onClick={() => navigate(`/cars/${car?.category_id}`)}
              >
                SEE ALL{" "}
                <span className="px-2  rounded-full border border-white">
                  {" "}
                  {">"}
                </span>
              </h1>
